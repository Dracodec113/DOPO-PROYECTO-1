...
public class TowerGrid {
    ...
    public TowerGrid(int heightCells, int widthCells) {
        ...
        this.grid = new StackingItem[rows][cols];
        clearGrid();
    }
    ...
    public boolean rebuild(ArrayList<String> order, HashMap<String, StackingItem> items) {
            clearGrid(); 
    ...
    private boolean canFitDeeper(int r, int c, int w, int h, StackingItem item) {
        ...
        for (int i = r; i < r + h; i++) {
            for (int j = c; j < c + w; j++) {
                ...
                StackingItem occupant = grid[i][j];
    ...
    private boolean isCupFloor(StackingItem item, int r, int c) {
        if (item == null || item instanceof Lid) return false;
    ...
    private void fillGrid(int r, int c, int w, int h, StackingItem item) {
        for (int i = r; i < r + h; i++) {
            for (int j = c; j < c + w; j++) {
                if (inGrid(i, j)) {
    ...
    public void render() {
        HashSet<StackingItem> drawn = new HashSet<>();
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                StackingItem item = grid[r][c];
    ...
    public StackingItem getCell(int row, int col) {
